	...
	char pPick;
		initialize(&s1);initialize(&s2);initialize(&res);
			do{
				...
				gotoxy(31,25);printf("Choice: ");scanf("%d",&choice);
			
				switch(choice){
					case 1:
						gotoxy(65,3);printf("Choice: ");gets(pPick);
						if (pPick == 'a' || pPick == 'A'){
							initialize(&s1);
							gotoxy(65, 5);printf("How many digits do you want to enter? ");scanf("%d", &limit);
							if (limit<=10){
								for (i=0; i<limit; i++){
